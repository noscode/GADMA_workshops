...
def hwe_ternary_plot(vcf_file, popmap_file, population_name, output_file=None, double_ids=True):
    ...
    # Draw triangle
    triangle = Polygon([[-1/np.sqrt(3),0], [0,1], [1/np.sqrt(3),0]], closed=True, edgecolor='black', fill=False, lw=2, zorder=9)
    ...
    # --- Points, colorbar ---
    scat = ax.scatter(tx, ty, s=30, c=color, cmap='plasma', marker='o', edgecolors='black', linewidth=0.1, alpha=1, zorder=10)
